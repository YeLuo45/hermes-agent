...
2. **base 必须匹配仓库名** — 部署到 `https://user.github.io/repo/` 时，`base: "/repo/"`（末尾斜杠不能少）
...
6. **不要在 job 内声明 environment** — workflow mode 部署时，`environment:` 块会触发环境保护规则，导致 main 分支被阻止部署。去掉 `environment:` 块，或在仓库 Settings → Environments 中预先配置
7. **REST API 推送用 gh api --input** — urllib.request 不走 gh auth，会 401。用 `gh api --input` 或 `subprocess + gh auth token` 代替
